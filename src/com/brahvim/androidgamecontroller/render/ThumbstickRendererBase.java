...
public class ThumbstickRendererBase {
    ...
    public static void displayDraw(@NotNull PGraphics p_graphics,
                                   @NotNull PVector p_draggingTouch, float p_x, float p_y,
                                   float p_scaleX, float p_scaleY, float innerCircleScale,
                                   boolean p_pressed) {
        // region The thumbstick's border ring.
        p_graphics.pushMatrix();
        ...
        p_graphics.translate(
          // HA-HA! You can't make these elliptical now... yippee! ^^:
          PApplet.constrain(p_draggingTouch.x - p_x, 0, p_x),
          PApplet.constrain(p_draggingTouch.y - p_y, 0, p_y));
        ...
        // Should there be a... `configChanged(p_oldConfig, p_newConfig)`
        // event so this can track the magnitude?:
        p_graphics.scale(innerCircleScale, innerCircleScale);

        p_graphics.noStroke();
        p_graphics.fill(220, 255, 220, p_pressed? 120 : 60);
        p_graphics.ellipse(0, 0, 1, 1);
